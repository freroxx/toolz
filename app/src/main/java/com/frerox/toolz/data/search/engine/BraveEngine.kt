...
class BraveEngine @Inject constructor(private val fetcher: JsoupFetcher) : SearchEngine {
    override val id = EngineId.BRAVE
    ...
    override fun buildSearchUrl(request: SearchRequest): List<String> {
        ...
        return when (request.category) {
            SearchCategory.IMAGES -> listOf("https://search.brave.com/images?q=$enc$safe")
            SearchCategory.NEWS -> listOf("https://search.brave.com/news?q=$enc$safe")
            SearchCategory.VIDEOS -> listOf("https://search.brave.com/videos?q=$enc$safe")
            else -> listOf("https://search.brave.com/search?q=$enc&source=web$safe$offset", "https://search.brave.com/search?q=$enc")
        }
    }
    override fun parse(html: String, baseUrl: String, request: SearchRequest): List<SearchResult> {
        return parseDocument(org.jsoup.Jsoup.parse(html, baseUrl), request)
    }
    ...
    override suspend fun search(request: SearchRequest): SearchResponse {
        for (url in buildSearchUrl(request)) {
            ...
            if (parsed.isNotEmpty()) return SearchResponse(parsed, OffsetBasedPagination.nextOffset(request.offset, request.pageSize, parsed.size, parsed.size>=10), parsed.size>=5, id)
        }
