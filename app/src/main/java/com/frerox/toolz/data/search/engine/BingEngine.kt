...
class BingEngine @Inject constructor(private val fetcher: JsoupFetcher) : SearchEngine {
    ...
    override fun buildSearchUrl(request: SearchRequest): List<String> {
        ...
        return when(request.category){
            SearchCategory.IMAGES -> listOf("https://www.bing.com/images/search?q=$enc$offset$safe")
            SearchCategory.NEWS -> listOf("https://www.bing.com/news/search?q=$enc$offset$safe")
            SearchCategory.VIDEOS -> listOf("https://www.bing.com/videos/search?q=$enc$offset$safe")
            else -> listOf("https://www.bing.com/search?q=$enc$offset$safe")
        }
    }
