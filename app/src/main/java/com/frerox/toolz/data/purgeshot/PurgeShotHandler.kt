...
object PurgeShotHandler {
    ...
    suspend fun handleNewScreenshot(
        ...
        // Not smart-auto: try to show popup. If app is in background, BAL will block activity -> fallback to notification
        val canShowPopup = ToolzApplication.isFocused.value || canDrawOverlays(context) || Build.VERSION.SDK_INT < Build.VERSION_CODES.Q
        if (canShowPopup) {
            try {
                ...
                Log.w(TAG, "Popup BAL blocked, fallback to notification", e)
            }
        }
        // Fallback: notification with timer actions (works outside Toolz 100%)
