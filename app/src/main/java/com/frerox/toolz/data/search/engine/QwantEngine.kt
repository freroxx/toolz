...
class QwantEngine @Inject constructor(private val fetcher: JsoupFetcher): SearchEngine{
    ...
    override val displayName="Qwant"
    ...
    override fun parse(html: String, baseUrl: String, request: SearchRequest): List<SearchResult>{
        ...
        doc.select("[data-testid=result], .result, article").forEachIndexed{ rank, el->
            val titleEl=el.select("a[href] h2, a[href] h3, a.result-title").firstOrNull() ?: el.select("a[href]").firstOrNull() ?: return@forEachIndexed
            ...
            val title=titleEl.text().trim().ifBlank{ fetcher.safeHost(cleanUrl)}
            results+=SearchResult(title, snippet, cleanUrl, fetcher.safeHost(cleanUrl), "Qwant", date, null, null, rank)
        }
        return results
    }
