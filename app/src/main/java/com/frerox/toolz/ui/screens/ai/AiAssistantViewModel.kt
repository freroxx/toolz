...
data class AiAssistantUiState(
    ...
    // ── New fields ───────────────────────────────────────────
    /** Chat summary generated by Groq on demand. */
    val chatSummary       : String?          = null,
    val isSummarizing     : Boolean          = false,
    /** True while generating or refreshing the chat title via AI. */
...
class AiAssistantViewModel @Inject constructor(
    ...
    fun updateIcon(icon: String)   { _settingsUiState.update { it.copy(selectedIcon = icon) } }
    ...
    fun saveConfig(name: String) {
        with(_settingsUiState.value) {
            settingsManager.saveConfig(AiConfig(name = name, provider = provider, model = selectedModel, apiKey = apiKey, iconRes = selectedIcon), editingConfig?.name)
        }
    ...
    fun sendMessage(text: String) {
        ...
        activeInferenceJob = viewModelScope.launch {
            ...
            if (currentId == null) {
                // Create chat with a placeholder title — AI will rename it shortly
    ...
    /**
     * Asks Groq to generate a short descriptive title for the chat based on the
     * first user message and AI reply. Called automatically after the first exchange.
     * Can also be triggered manually by the user via [refreshChatTitle].
     */
    ...
    /**
     * Regenerates the chat title using the full conversation history.
     * Called when the user presses the refresh-title button.
     */
    ...
    /**
     * Generates a concise bullet-point summary of the current conversation using
     * Groq llama-3.3-70b-versatile. Result stored in [AiAssistantUiState.chatSummary].
     */
    fun summarizeChat() {
        val messages = _uiState.value.messages
        if (messages.isEmpty()) return
